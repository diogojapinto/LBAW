...
function acceptProposal($username, $idDeal)
{
    ...
    $stmt = $conn->prepare("SELECT interactionNo, amount
                            FROM Interaction
                            WHERE idDeal = :idDeal
                              AND interactionno =
                                (SELECT MAX(interactionno)
                                FROM interaction
                                WHERE iddeal = :idDeal);");
    ...
    $result = $stmt->fetch();
    $lastInteraction = $result['interactionno'];
...
function getDealState($idDeal)
{
    ...
    switch ($dealState) {
        case "Pending":
            ...
            $stmt = $conn->prepare("SELECT interactionNo, amount, interactiontype
                            FROM Interaction
                            WHERE idDeal = :idDeal
                              AND interactionno =
                                (SELECT MAX(interactionno)
