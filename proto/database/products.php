...
function insertProduct($name, $description, $category)
{
    ...
    // product approved by default
    $stmt = $conn->prepare("INSERT INTO Product(name, description, approved)
...
function updateBuying($idProduct, $username, $proposedPrice)
{
    global $conn;

    if (isBuyer($username)) {
        $id = getIdUser($username);
        $stmt = $conn->prepare("UPDATE WantsToBuy
                                SET proposedprice = :proposedPrice
